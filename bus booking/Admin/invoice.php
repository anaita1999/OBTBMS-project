...
class Payments extends BasePayments
{
    public static function grid($rowNum = 10)
    {
        ...
        $columns[] = array('label' => $translator->translate('Expense'), 'field' => 'p.outcome', 'alias' => 'outcome', 'sortable' => true, 'type' => 'float', 'searchable' => true);
        ...
        $config['datagrid']['dqrecordset'] = Doctrine_Query::create()->select($config['datagrid']['fields'])->from('Payments p')->leftJoin('p.Customers c')->leftJoin('c.Customers r')->leftJoin('p.Banks b')->leftJoin('p.Orders o')->leftJoin('o.Invoices i')->leftJoin('o.Statuses s')->orderBy('paymentdate desc');
    ...
    {
        try {
            $dq = Doctrine_Query::create()->from('Payments p2')->leftJoin('p2.Orders o')->leftJoin('o.Isp i')->leftJoin('o.Invoices in')->leftJoin('o.OrdersItems oi')->leftJoin('oi.BillingCycle bc')->leftJoin('o.OrdersItemsDomains oid')->leftJoin('oid.Domains d')->leftJoin('d.DomainsTlds tld')->leftJoin('tld.WhoisServers w')->leftJoin('oi.Products p')->leftJoin('p.Taxes t')->leftJoin('o.Customers c')->leftJoin('c.Addresses a')->leftJoin('a.Countries co')->leftJoin('o.Statuses s')->where("payment_id = ?", $id)->limit(1);
            if ($fields != "*") {
                $dq->select($fields);
            }
    ...
    {
        $dq = Doctrine_Query::create()->select($fields)->from('Payments p')->where("p.payment_id = {$id}")->limit(1);
        $retarray = $retarray ? Doctrine_Core::HYDRATE_ARRAY : null;
    ...
    {
        $record = Doctrine_Query::create()->select('payment_id, 
        						  ...
        						  o.total as total,
        						  o.vat as vat,
        						  o.grandtotal as grandtotal')->from('Payments p')->leftJoin('p.Orders o')->leftJoin('o.Customers c')->leftJoin('o.Invoices i')->leftJoin('p.Banks b')->where('YEAR(o.order_date) = ?', $year)->andWhere('MONTH(o.order_date) = ?', $month)->andWhere('o.invoice_id is not null')->andWhere('o.status_id = ?', Statuses::id("complete", "orders"))->execute(array(), Doctrine_Core::HYDRATE_ARRAY);
        return $record;
    }
    /**
     * Save the record
    ...
    {
        $dq = Doctrine_Query::create()->from('Payments p')->leftJoin('p.Banks b')->where("p.order_id = ?", $order_id)->orderby('p.paymentdate DESC');
        //->limit ( 1 );
        if ($fields != "*") {
            $dq->select($fields);
        }
    ...
    {
        try {
            Doctrine_Query::create()->update('Payments p')->set('p.confirmed', '1')->where('p.order_id = ?', intval($orderid))->execute();
    ...
    {
        return Doctrine_Query::create()->select($fields)->from('Payments p')->leftJoin('p.Orders o')->leftJoin('o.Invoices i')->leftJoin('o.Customers c')->leftJoin('c.Customers r')->leftJoin('o.Statuses s')->whereIn("payment_id", $ids)->orderBy(!empty($orderby) ? $orderby : "")->execute(array(), Doctrine::HYDRATE_ARRAY);
    }
    /**
    ...
    {
        if (is_numeric($id)) {
            $record = Doctrine_Query::create()->select('order_id')->from('Payments p')->whereIn("payment_id", $id)->execute(array(), Doctrine::HYDRATE_ARRAY);
